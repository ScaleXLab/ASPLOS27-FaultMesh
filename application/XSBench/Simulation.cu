                           ...
                           int mype, double *kernel_time)
{
  ...
  cudaMallocManaged((void**)&num_nucs_d, sizeof(int) * SD.length_num_nucs);
  memcpy(num_nucs_d, SD.num_nucs, sizeof(int) * SD.length_num_nucs);

  //buffer<double, 1> concs_d(SD.concs, SD.length_concs);
  cudaMallocManaged((void**)&concs_d, sizeof(double) * SD.length_concs);
  memcpy(concs_d, SD.concs, sizeof(double) * SD.length_concs);

  //buffer<int, 1> mats_d(SD.mats, SD.length_mats);
  cudaMallocManaged((void**)&mats_d, sizeof(int) * SD.length_mats);
  memcpy(mats_d, SD.mats, sizeof(int) * SD.length_mats);

  //buffer<NuclideGridPoint, 1> nuclide_grid_d(SD.nuclide_grid, SD.length_nuclide_grid);
  cudaMallocManaged((void**)&nuclide_grid_d, sizeof(NuclideGridPoint) * SD.length_nuclide_grid);
  memcpy(nuclide_grid_d, SD.nuclide_grid, sizeof(NuclideGridPoint) * SD.length_nuclide_grid);

  //buffer<int, 1> verification_d(verification_h, in.lookups);
  cudaMallocManaged((void**)&verification_d, sizeof(int) * in.lookups);
  memset(verification_d, 0, sizeof(int) * in.lookups);
  ...
  cudaMallocManaged((void**)&unionized_energy_array_d, sizeof(double) * SD.length_unionized_energy_array);
  memcpy(unionized_energy_array_d, SD.unionized_energy_array, 
  ...
  cudaMallocManaged((void**)&index_grid_d, sizeof(int) * (unsigned long long)SD.length_index_grid);
  memcpy(index_grid_d, SD.index_grid, sizeof(int) * (unsigned long long )SD.length_index_grid);
  ...
  ////////////////////////////////////////////////////////////////////////////////
  if (getenv("FAULT_RATIO") && strcmp(getenv("FAULT_RATIO"), "0") == 0) {
    int dev = 0;
    cudaGetDevice(&dev);
    cudaMemPrefetchAsync(num_nucs_d, sizeof(int) * SD.length_num_nucs, dev);
    cudaMemPrefetchAsync(concs_d, sizeof(double) * SD.length_concs, dev);
    cudaMemPrefetchAsync(mats_d, sizeof(int) * SD.length_mats, dev);
    cudaMemPrefetchAsync(nuclide_grid_d, sizeof(NuclideGridPoint) * SD.length_nuclide_grid, dev);
    cudaMemPrefetchAsync(verification_d, sizeof(int) * in.lookups, dev);
    cudaMemPrefetchAsync(unionized_energy_array_d, sizeof(double) * SD.length_unionized_energy_array, dev);
    cudaMemPrefetchAsync(index_grid_d, sizeof(int) * (unsigned long long)SD.length_index_grid, dev);
  }
...
//   for (int i = 0; i < in.kernel_repeat; i++) {
  ...
  *kernel_time = (kstop - kstart) / in.kernel_repeat;
  ...
  memcpy(verification_h, verification_d, sizeof(int) * in.lookups);
