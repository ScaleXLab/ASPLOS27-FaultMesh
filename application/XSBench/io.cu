                   ...
                   unsigned long long vhash, double kernel_time )
{
...
#else
    printf("Total Time Statistics (Device Init / JIT Compilation + Simulation Kernel)\n");
    printf("Runtime:               %.3lf seconds\n", runtime);
    printf("Lookups:               ");fancy_int(lookups);
    printf("Lookups/s:             ");
    fancy_int(lookups_per_sec);
    printf("Simulation Kernel Only Statistics\n");
    printf("Average kernel execution time: %.3lf seconds\n", kernel_time);
    printf("\nGPU Runtime: %.6lfs\n", kernel_time);
    printf("Lookups/s:             ");
  ...
  if( in.simulation_method == EVENT_BASED )
  {
    small = 945990;
    large = 952131;
    // The CUDA port currently defaults to the nuclide-grid event-based path
    // rather than the older unionized-grid configuration used by the original
...
void print_CLI_error(void)
{
  ...
  printf("  -r <kernel count>        Specifies the kernel execution count. (1 is default.)\n");
...
Inputs read_CLI( int argc, char * argv[] )
{
  ...
  input.n_gridpoints = 248847;

  // defaults to 500,000
  input.particles = 500000;

  // defaults to 34
  input.lookups = 170000;
  ...
  // defaults to kernel execution once 
  input.kernel_repeat = 1;
  ...
  for( int i = 1; i < argc; i++ )
  {
    ...
    else if( strcmp(arg, "-r") == 0 )
    {
      if( ++i < argc )
      {
        input.kernel_repeat = atoi(argv[i]);
      }
      else
        print_CLI_error();
    }
